...
class RemoveSubscriptionTask extends SchedulerTask
{
    ...
    {
        return 'remove waiting payment subscription task';
    }
    ...
    public function run()
    {
        ...
        $subs = $doctrine->getRepository('AppBundle:Subscription')->getSubscriptionsToRemove();

        foreach ($subs as $sub)
        {
            $this->output->writeln('Processing subscription #'.$sub->getId());
            $doctrine->getManager()->remove($sub);
        }
